...
class Ebizmarts_Autoresponder_Model_EventObserver
{
    ...
    public function orderSaved(Varien_Event_Observer $observer){
        ...
        if (isset($new_data['status']) && isset($original_data['status']) && $new_data['status'] && $original_data['status'] !== $new_data['status'] && $new_data['status'] == Mage::getStoreConfig(Ebizmarts_Autoresponder_Model_Config::NEWORDER_ORDER_STATUS, $storeId)) {
