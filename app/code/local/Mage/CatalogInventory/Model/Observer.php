...
class Mage_CatalogInventory_Model_Observer
{
        ...
        if ($options && $qty > 0) {
            ...
            foreach ($options as $option) {
                ...
                if ($result->getHasError()) {
                    ...
                    $controllo=$this->_controlloDisponibilitaWS($quoteItem->getProductId(),$qty);
    ...
    protected function _controlloDisponibilitaWS($id,$qty){
        if (isset($_SERVER["REQUEST_URI"])){
            $uri=$_SERVER["REQUEST_URI"];
            $posizione=strpos($uri, "onestepcheckout");
        }
        else {
            $posizione=false;
        }
        ...
        if ($posizione){
            ...
            if (isset($username) && $username != "" && isset($password) && $password != "" && isset($service_url) && $service_url != "") {
                ...
                if ($product->getTypeId() == "simple") {
                    ...
                    $misuraMage = $product->getAttributeText("ca_misura");
                    ...
                    if ($idProdottoConfigurabile[0] != "") {
                        $productConfigurabile = Mage::getModel('catalog/product')->load($idProdottoConfigurabile[0]);
                        $skuConfigurabile = $productConfigurabile->getSku();
                        ...
                        $service_urlGet = $service_url . "/stocks?id=" . $skuConfigurabile;
                        $curlGet = curl_init($service_urlGet);
                        curl_setopt($curlGet, CURLOPT_HTTPAUTH, CURLAUTH_ANY); // accetto ogni autenticazione. Il sistema utilizzerà la migliore
                            ...
                            for ($k = 0; $k < count($valoreStocks); $k++) {
                                ...
                                foreach ($varianti as $key => $value) {
                                    ...
                                    foreach ($misuraDispo as $key2 => $value2) {
                                        ...
                                        if (strcmp(strtolower($misura), strtolower($misuraMage)) == 0) {
                                            $qtyTot = $qtyTot + $disponibilita;
                                            $trovato = true;
                                        }
                                    }
