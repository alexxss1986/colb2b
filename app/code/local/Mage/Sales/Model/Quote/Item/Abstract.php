    ...
    implements Mage_Catalog_Model_Product_Configuration_Item_Interface
{
    ...
    public function calcRowTotal()
    {
        ...
        $rawTax= Mage::getSingleton('tax/calculation_rate')->loadByCode(Mage::getSingleton('checkout/type_onepage')->getQuote()->getShippingAddress()->getCountryId());
        $taxRate = intval($rawTax->getData("rate"));

        //if (number_format($this->_getData('tax_amount'),0,"","")==0){
        if ($taxRate==0){
            $total = round($this->getCalculationPriceOriginal(),2) * $qty;
            $baseTotal  = round($this->getBaseCalculationPriceOriginal(),2) * $qty;
        }
        else {
