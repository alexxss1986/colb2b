    ...
    if ($_SESSION['livello']==3) {
            ...
            isset($_SESSION['scalari_s']))
        {
            ...
            for ($k = 0; $k < count($immagini); $k++) {
                $punto = strrpos($immagini_nome[$k], ".");
                $file_new = substr($immagini_nome[$k], 0, $punto);
            ...
            if ($controlloImg==true) {
                ...
                $esiste = Mage::getModel('catalog/product')->getIdBySku($sku);

                // controllo se il prodotto configurabile esiste
                if ($esiste == "") {
                    ...
                    for ($i = 0; $i < count($taglie); $i++) {
                        ...
                        $array_sku[$i] = $sku_semplice; //inserisco in un array tutti gli sku inseriti
                        ...
                        $newids[$productSimple->getId()] = 1;

                    }

                    $productConfigurable = Mage::getModel('catalog/product');
                    $productConfigurable->setSku($sku);
                    ...
                    $productConfigurable->setWebsiteIds(array(2));
                    ...
                    for ($k = 0; $k < count($immagini); $k++) {
                        $nome_immagine2 = $immagini_nome[$k];
                        $link = "../../var/images/" . $nome_immagine2;
                        unlink($link);
                    }
                    ...
                    echo "<script>alert('Errore. Lo sku inserito è già presente in magazzino. Se vuoi inserire una nuova taglia e colore selezionare \"Aggiorna Prodotto\"');location.replace('../index.php')</script>";
                }
            }
            else {
                echo "<script>alert('Non è stata inserita l\\'immagine principale!');location.replace('../index.php')</script>";
            }
